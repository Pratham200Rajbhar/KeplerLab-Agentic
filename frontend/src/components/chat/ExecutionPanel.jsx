...
import { executeCode } from '@/lib/api/chat';
      ...
      while (true) {
        ...
        for (const part of parts) {
          ...
          try {
            ...
            if (eventName === 'stdout') setOutput(prev => prev + payload.line + '\n');
            else if (eventName === 'result') {
              setExitCode(payload.exit_code ?? 0);
              if (payload.stderr) setOutput(prev => prev + '\n[stderr]\n' + payload.stderr);
    ...
    <div className="execution-panel">
      <div className="execution-header">
        <div className="execution-header-left">
          <span className="execution-icon"><FileText size={14} /></span>
          <span className="execution-title">Generated Script</span>
          <span className="execution-lang">python</span>
        </div>
        <button className="execution-collapse-btn" onClick={() => setCollapsed(v => !v)} title={collapsed ? 'Expand' : 'Collapse'}>
      ...
      <div className="execution-actions">
        <button className={`execution-run-btn ${running ? 'execution-run-btn--running' : ''}`} onClick={handleRun} disabled={running}>
          {running ? <><span className="execution-spinner" />Running...</> : '▶ Run Script'}
        </button>
        {exitCode !== null && (
          <span className={`execution-exit-badge ${exitCode === 0 ? 'exit-ok' : 'exit-err'}`}>Exit: {exitCode}</span>
        )}
      </div>
      {(output || running) && (
        <div className="terminal-box" ref={terminalRef}>
          <div className="terminal-header">
            <span className="terminal-icon">📟</span><span>Terminal Output</span>
