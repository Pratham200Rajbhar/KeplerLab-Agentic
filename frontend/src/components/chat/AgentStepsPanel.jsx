...
const INTENT_COLORS = {
  QUESTION: 'intent-question', DATA_ANALYSIS: 'intent-data', RESEARCH: 'intent-research',
  CODE_EXECUTION: 'intent-code', CONTENT_GENERATION: 'intent-content', UNKNOWN: 'intent-unknown', ERROR: 'intent-error',
};
const INTENT_ICONS = {
  QUESTION: '🤔', DATA_ANALYSIS: '📊', RESEARCH: '🔬', CODE_EXECUTION: '⚙️',
  CONTENT_GENERATION: '✨', UNKNOWN: '❓', ERROR: '⚠️',
};
...
export default function AgentStepsPanel({ meta }) {
  ...
  return (
    <div className="agent-steps-panel">
      <button className="agent-steps-toggle" onClick={() => setExpanded(v => !v)} aria-expanded={expanded}>
        ...
        <span className={`intent-badge ${INTENT_COLORS[intent] || 'intent-unknown'}`}>{INTENT_ICONS[intent]} {intent}</span>
