...
export default function SuggestionDropdown({ suggestions, loading, onSelect, onClose }) {
  ...
  const handleKeyDown = useCallback(e => {
    ...
    else if (e.key === 'Enter' && activeIndex >= 0) { e.preventDefault(); if (suggestions[activeIndex]) onSelect(suggestions[activeIndex].suggestion); }
        ...
        {suggestions.map((item, idx) => (
          <li key={item.suggestion} className={`suggestion-item ${idx === activeIndex ? 'suggestion-item-active' : ''}`}
            ...
            <div className="confidence-bar-wrapper">
              <div className="confidence-bar-fill" style={{ width: `${Math.round((item.confidence || 0) * 100)}%` }} />
            </div>
          </li>
        ))}
      </ul>
