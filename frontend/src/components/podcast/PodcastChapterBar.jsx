  ...
  if (!chapters.length) {
    return (
      <div className="px-4 py-8 text-center">
        <BookOpen className="w-5 h-5 text-[var(--text-muted)] mx-auto mb-2 opacity-40" />
        <p className="text-xs text-[var(--text-muted)]">No chapters available</p>
      </div>
    );
  }
        ...
        return (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              {ch.summary && (
                <p className="text-[10px] text-[var(--text-muted)] truncate mt-0.5">{ch.summary}</p>
              )}
            </div>

            {hasBookmark && (
              <Bookmark className="w-3 h-3 text-[var(--accent)] shrink-0" fill="currentColor" />
            )}
          </button>
        );
      })}
