...
async def generate_skill_draft(
    ...
    generation_prompt = (
        ...
        "- For categorical variables, append options: `<name>: {var} [Option1, Option2]`\n"
        "- For almost all free-form text inputs (like topics, concepts, material), append type: `<name>: {var} (textarea)`\n"
        "- Ensure all {variables} used in steps are defined in the `## Input` block.\n"
        "- You MUST include the `## Input` block exactly as shown.\n"
    ...
    try:
        ...
        try:
            ...
            # Auto-recover missing variables rather than discarding the entire LLM generated skill
            declared = {inp.name for inp in definition.inputs}
            declared.add("user_input")
            undefined = sorted(v for v in definition.all_variables if v not in declared)
            if undefined:
                injection = "\n" + "\n".join(f"{v}: {{{v}}} (textarea)" for v in undefined)
                if "## Input" in generated:
                    generated = generated.replace("## Input", "## Input" + injection, 1)
                else:
                    generated = re.sub(r"(# Skill:.*?)\n", r"\1\n\n## Input" + injection + "\n", generated, count=1)
                
                # Re-parse with injected variables
                definition = parse_skill_markdown(generated)
                _validate_variables_defined(definition)
                logger.info("Auto-recovered %d undefined variables in generated skill draft.", len(undefined))

    except Exception as e:
        logger.warning("Skill generated by LLM was invalid even after recovery: %s", e)
