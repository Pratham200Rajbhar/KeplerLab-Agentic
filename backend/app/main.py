...
async def lifespan(app: FastAPI):
    ...
    try:
        import glob
        import shutil
        stale_pattern = os.path.join(settings.DATA_TMP_DIR, "kepler_*")
        stale_dirs = glob.glob(stale_pattern)
        if stale_dirs:
            for d in stale_dirs:
                if os.path.isdir(d):
                    shutil.rmtree(d, ignore_errors=True)
            logger.info("Cleaned up %d stale temp directories in %s", len(stale_dirs), settings.DATA_TMP_DIR)
