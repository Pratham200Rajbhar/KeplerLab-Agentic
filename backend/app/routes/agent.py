...
class RunGeneratedRequest(BaseModel):
    """User-approved execution of LLM-generated code (/code slash command).

    The code field must arrive from a prior code_generation_tool invocation.
    Never accept raw user input here — code must have been generated by the LLM.
    """
...
# ── /run-generated — user-approved execution of LLM-generated code ──

@router.post("/run-generated")
async def run_generated_code_endpoint(
    request: RunGeneratedRequest,
    current_user=Depends(get_current_user),
):
    """Execute LLM-generated code after the user explicitly approves it.

    This is the ONLY valid execution path for the /code slash command.
    The user must click [Run in Sandbox] in the CodeReviewBlock UI — execution
    is never triggered automatically.

    SSE events: start → stdout → result/error → done
    """
    from app.services.code_execution.executor import execute_code
    import asyncio

    # Only Python is sandbox-executable for now; other languages get a clear error.
    if request.language.lower() not in ("python", "py"):
        async def _unsupported():
            yield f"event: error\ndata: {json.dumps({'error': f'Sandbox supports Python only; got {request.language!r}'})}\n\n"
            yield f"event: done\ndata: {json.dumps({})}\n\n"
    ...
    async def execute_task():
        try:
            ...
            logger.error("run-generated execute task failed: %s", e)
            await queue.put({"type": "error", "error": str(e)})
        ...
        try:
            ...
            while True:
                ...
                if msg["type"] == "stdout":
                    ...
                    yield f"event: result\ndata: {json.dumps({**msg['data'], 'elapsed': elapsed})}\n\n"
                    break
                elif msg["type"] == "error":
                    yield f"event: error\ndata: {json.dumps({'session_id': session_id, 'error': msg['error']})}\n\n"
                    break
        except Exception as e:
            logger.error("run-generated event_generator failed: %s", e)
            yield f"event: error\ndata: {json.dumps({'error': str(e)})}\n\n"
        finally:
            elapsed = round(time.time() - start_ts, 2)
            yield f"event: done\ndata: {json.dumps({'session_id': session_id, 'elapsed': elapsed})}\n\n"
            if not task.done():
                task.cancel()
